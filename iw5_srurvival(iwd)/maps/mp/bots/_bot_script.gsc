...
connected()
{
	...
	level.currentBotCount++;//survival
	self thread onSpawned();//edited for survival

	self thread onDeath();//edited for survival

	//self thread onGiveLoadout(); //removed for survival class testing
...
//new concept
onDeath() //this SHOULD make it so bots have lifes, and get kicked after they are up
{
    self endon("disconnect");
	bot = self getEntityNumber();
	//self.canspawn = 0;
    ...
    for(;;)
    {
		...
		if( round <= 4 ) //should not need this
		{
			self.canspawn = 1;
		}
		else if( round <= 8 )
		...
		//spawn handler
		if( round <= 4 ) //not needed on low rounds
		{
			self.deaths = 0; 
			kick( bot );
			continue;
		}
		if(self.canspawn == self.deaths) //5+ spawer
		{
			self.deaths = 0;
			kick( bot );
			continue;
...
onSpawned()
{
	...
	for ( ;; )
	{
		...
		//level.botsLeftToSpawn--; < was this
		if (level.botsLeftToSpawn - 1 >= 0)
		{
			level.botsLeftToSpawn--;//survival
		}
		
		//bots to be kiced where spawning and decrenting the count to below 0 so they wasnt getting kicked 

		//set ai type
		self thread create_typed_ai();

		if ( randomInt( 100 ) <= self.pers["bots"]["behavior"]["class"] )
			self.bot_change_class = undefined;

		self.bot_lock_goal = false; //try setting true to stop bot wonder
...
/*
	Gets types that are in wave, sets type orrcordingly.
*/
create_typed_ai()
{
	//isType is used for weapon handles, its set to default then the class the bot needs to fill will be picked
		self.isType = "default";
	
		if (level.LightTroopsBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.LightTroopsBotsLeftToSpawn--;
				self.isType = "isLight";
				//self setmodel("");
			}
		}
		
		if (level.MediumTroopsBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.MediumTroopsBotsLeftToSpawn--;
				self.isType = "isMedium";
				//self setmodel("");
			}
		}

		if (level.HeavyTroopsBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.HeavyTroopsBotsLeftToSpawn--;
				self.isType = "isHeavy";
				//self setmodel("");
			}
		}
		
		if (level.CommandoTroopsBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.CommandoTroopsBotsLeftToSpawn--;
				self.isType = "isCommndo";
				//self setmodel("");
			}
		}
		
		if (level.HeavyCommandoTroopsBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.HeavyCommandoTroopsBotsLeftToSpawn--;
				self.isType = "isHeavyCommando";
				//self setmodel("");
			}
		}
		
		if(level.suicideBomberBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.suicideBomberBotsLeftToSpawn--;
				self.v["damage"] = 200;
				self.isType = "isBomber";
				self thread scripts\_survival_round_util::create_bomber();
				//self setmodel("");
			}
		}
		
		if (level.juggBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.juggBotsLeftToSpawn--;
				self.isType = "isJuggernaut";
				self.isjuggernaut = 1;
				thread maps\mp\_utility::recipeclassapplyjuggernaut();
				self setmodel("mp_fullbody_opforce_juggernaut");
				//jugg_heli_jump();
				//self waittill("heli_jump_done");
				self notify("spawned_player"); //bypass the respawn needed in apply jugg func
			}
		}
		if (level.juggRiotBotsLeftToSpawn - 1 >= 0)
		{
			if(self.isType == "default")
			{
				level.juggBotsLeftToSpawn--;
				self.isType = "isJuggernaut_riot";
				self.isjuggernaut = 1;
				thread maps\mp\_utility::recipeclassapplyjuggernaut();
				self setmodel("mp_fullbody_opforce_juggernaut");
				//jugg_heli_jump();
				//self waittill("heli_jump_done");
				self notify("spawned_player"); //maybe I can bypass the respawn
			}
		}
		
		//survival class main func call (test)
		self thread survival_class_think(self.isType);
}

/*
	This is the function that should handle how the jugg heli jump works (not added)
*/
jugg_heli_jump()
{
	/*
	...
	{
		case"mp_dome":
			heli_waypoint = (0,0,0); //where the heli is brought to hover
			hide_jugg_cords = (0,0,0); //where the jugg is teliported while waiting
			groundzone_cords = (0,0,0); //where jugg will be when they finsh jump
		break;
		
	}
	if(self.isType == "isJuggernaut" || self.isType == "isJuggernaut_riot")
	{
		//teliport them somewhere in the map where no one can see em
	}
	spawn heli then bring it to set cords on each map
	if(heli_name.origin == heli_waypoint)
	{
		self notify("jump_started");
	}
	if(self.origin == groundzone_cords)
	{
		self notify("heli_jump_done");
	}
	*/
...
/*
	When a bot spawns, they get a class type, this will choose thier loadout
*/
survival_class_think(class_type)
{
	self takeallweapons(); //make sure they have nothing
	self.changingWeapon = undefined; //make sure they dont auto change weapons during this process
	secondaryName = "none";
	primaryName = "none";
	
	if(isDefined(class_type)) //cheak if they have a class type
	{
		switch(class_type)
		{
			case"isBomber":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_usas12_mp";
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				//allClientsPrint("Bomber Troop Choosen!");
			break;
			case"isLight":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_1887_mp";
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				//allClientsPrint("Light Troop Choosen!");
			break;
			case"isMedium":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_mp5_mp";
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				//allClientsPrint("Med Troop Choosen!");
			break;
			case"isHeavy":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_ak47_mp";
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				//allClientsPrint("Heavy Troop Choosen!");
			break;
			case"isCommndo":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_acr_mp";
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				//allClientsPrint("C Troop Choosen!");
			break;
			case"isHeavyCommando":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_fad_mp";
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				//allClientsPrint("H C Troop Choosen!");
			break;
			case"isJuggernaut":
				secondaryName = "none";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_m60jugg_mp"; //for riots use "iw5_riotshieldjugg_mp"
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				allClientsPrint("Juggernaut Choosen!");
			break;
			case"isJuggernaut_riot":
				secondaryName = "none";
				self _giveWeapon( secondaryName );
				primaryName = "iw5_riotshieldjugg_mp"; //for riots use "iw5_riotshieldjugg_mp"
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				allClientsPrint("Juggernaut Riot Choosen!");
			break;
			case"default":
				secondaryName = "iw5_usp45_mp";
				self _giveWeapon( secondaryName );
				...
				self _giveWeapon( primaryName );
				self switchtoweapon( primaryName );
				allClientsPrint("Class Type default: Defaulting to Light Troop");
			break;
		}
	}
	else //no class type, default to a non true light troop.
	{
		secondaryName = "iw5_usp45_mp";
		self _giveWeapon( secondaryName );
		primaryName = "iw5_1887_mp";
		self _giveWeapon( primaryName );
		self switchtoweapon( primaryName );
		allClientsPrint("No Class Type Choosen: Defaulting to Light Troop");
	}
}
