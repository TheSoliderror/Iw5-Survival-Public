...
/*
	converts a string into a float_str
*/
float_str( num )
{
...
parseTokensIntoWaypoint( tokens )
{
	...
	waypoint.origin = ( float_str( orgToks[0] ), float_str( orgToks[1] ), float_str( orgToks[2] ) );
	...
	if ( isDefined( anglesStr ) && anglesStr != "" )
	{
		...
		waypoint.angles = ( float_str( anglesToks[0] ), float_str( anglesToks[1] ), float_str( anglesToks[2] ) );
	}
	...
	if ( isDefined( javStr ) && javStr != "" )
	{
		...
		waypoint.jav_point = ( float_str( javToks[0] ), float_str( javToks[1] ), float_str( javToks[2] ) );
	}
...
/*
	Patches giveLoadout so that it doesn't use IsItemUnlocked (I think this can be removed, keeping it rn just see how other things are done)
*/
botGiveLoadout( team, class, allowCopycat, setPrimarySpawnWeapon ) // setPrimarySpawnWeapon only when called during spawn
{
	...
	//This is where the bots get the new weapons, we need to add bot types now.
	if(!isDefined(self.isType))
	{
		self waittill("spawned_player");
		wait 1.0;
		allClientsPrint("No Type Choosen Attempting To Fix");
	}
	if(isDefined(self.isType))
	{
		switch(self.isType)
	...
	if ( var_7 )
	{
		self setweaponammoclip( self.primaryWeapon, 7 );
		self setweaponammostock( self.primaryWeapon, 21 );
	}
